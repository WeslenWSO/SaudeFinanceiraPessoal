#!/usr/bin/env python
"""
Cadastra serviços ausentes e importa tabela de preços CB Saúde / Bombeiro (Medicinarte).

TSV: codigo, nome, valor_sem_contraste, valor_com_contraste
  - valor_sem_contraste  -> preco_apartamento
  - valor_com_contraste  -> preco_enfermaria

  set DATABASE_URL=postgresql://...
  python scripts/importar_tabela_preco_bombeiro.py
  python scripts/importar_tabela_preco_bombeiro.py --dry-run
"""
...
CONVENIO_NOME = "CORPO DE BOMBEIRO"
CABECALHO_NOME = "TABELA - BOMBEIRO"
...
def main() -> int:
    ...
    if not args.arquivo.is_file():
        ...
        print("Execute: python scripts/gerar_tsv_cbsaude.py", file=sys.stderr)
        return 1
    ...
    if args.dry_run:
        for codigo, nome, vi, ve in linhas[:12]:
            print(f"  {codigo}\t{nome[:45]}\t{vi}\t{ve}")
        print("  ...")
        return 0
    ...
    if not convenio:
        alt = Convenio.objects.filter(empresa=empresa, nome__icontains="BOMBEIRO").first()
        convenio = alt
    if not convenio:
