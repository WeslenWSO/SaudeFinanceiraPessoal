#!/usr/bin/env python
"""
Importa tabela de preços PM (Medicinarte) vinculada aos ServicosMedicos.

  set DATABASE_URL=postgresql://...
  python scripts/importar_tabela_preco_pm.py
  python scripts/importar_tabela_preco_pm.py --dry-run
"""
...
CONVENIO_NOME = "POLICIA MILITAR - POLICLINICA"
CABECALHO_NOME = "TABELA - PM"
EMPRESA_ID_DEFAULT = 16
TSV_DEFAULT = Path(__file__).resolve().parent / "dados" / "tabela_preco_pm.tsv"
...
def main() -> int:
    ...
    print(f"Linhas: {len(linhas)} | Empresa: {args.empresa_id} | Cabeçalho: {CABECALHO_NOME}")
    ...
    if faltando:
        print(f"AVISO: códigos sem ServicosMedicos: {', '.join(faltando_lista)}", file=sys.stderr)
        return 1
    return 0
