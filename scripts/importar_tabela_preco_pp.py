#!/usr/bin/env python
"""
Cadastra serviços ausentes e importa tabela de preços PP Saúde (Medicinarte).

TSV: codigo, nome, valor_sem_contraste, valor_com_contraste

  set DATABASE_URL=postgresql://...
  python scripts/importar_tabela_preco_pp.py
  python scripts/importar_tabela_preco_pp.py --dry-run
"""
...
CONVENIO_NOME = "PP SAUDE"
CABECALHO_NOME = "TABELA - PP"
...
def main() -> int:
    ...
    if not args.arquivo.is_file():
        ...
        print("Execute: python scripts/gerar_tsv_pp.py", file=sys.stderr)
        return 1
    ...
    if not convenio:
        alt = Convenio.objects.filter(empresa=empresa, nome__icontains="PP").first()
        convenio = alt
    if not convenio:
